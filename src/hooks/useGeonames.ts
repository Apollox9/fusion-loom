...
const GEONAMES_USERNAME = 'demo'; // Using demo account - in production, use your own
...
export function useGeonames() {
  ...
  const fetchCountries = async () => {
    ...
    try {
      ...
      if (data.geonames) {
        setCountries(data.geonames.map((c: any) => ({
  ...
  const fetchRegions = async (countryCode: string) => {
    ...
    try {
      ...
      const data = await response.json();
      if (data.geonames) {
        setRegions(data.geonames.map((r: any) => ({
  ...
  const fetchDistricts = async (regionGeonameId: number, countryCode: string) => {
    ...
    try {
      ...
      if (data.geonames && data.geonames.length > 0) {
        setDistricts(data.geonames.map((d: any) => ({
